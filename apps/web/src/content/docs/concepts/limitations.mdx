...
VS Code solves this with layout-aware labels + warnings. Tactile keeps labels simpler — see [International keyboards](/concepts/international-keyboards/) for the convention to note in your help UI.
...
By default, shortcuts don't fire while typing in `input`, `textarea`, `select`, or `contenteditable`. Strategies for overriding this live in [Engine options → `ignore`](/core/engine-options/#ignore-event-keyevent--boolean).
