...
| `enableInFormFields` | `boolean` | `false` | Fire even while typing in inputs/textareas/contenteditable. Chords only — see [Engine options → `ignore`](/core/engine-options/#allow-one-shortcut-in-inputs-per-rule). |
...
[Engine options → `source`](/core/engine-options/#source-keyeventsource).
...
- [Engine options](/core/engine-options/) — every constructor option in depth
- [Key syntax](/concepts/key-syntax/) — everything `keys` accepts
- [Recipes](/guides/recipes/) — the API applied to palettes, help dialogs, and rebinding UIs
