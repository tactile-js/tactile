...
Default match strategy for rules that don't set their own `match`. Defaults to `'hybrid'`.

| Mode | Behavior |
| --- | --- |
| `'hybrid'` | Letters/digits → `event.code`; symbols/named keys → `event.key` |
| `'physical'` | Always `event.code` (symbols fall back to `event.key`) |
| `'logical'` | Always `event.key` |

See [Match modes](/concepts/matching/) and [International keyboards](/concepts/international-keyboards/).
