...
Three strategies — default ignore, per-shortcut `when` gating, and selective passthrough — with code
for each in [Engine options → `ignore`](/core/engine-options/#ignore-event-keyevent--boolean).
